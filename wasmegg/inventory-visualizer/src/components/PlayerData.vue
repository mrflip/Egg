...
<template>
  <div class="flex flex-row w-full max-w-[1400px] m-auto justify-center">
    ...
    <div>
      ...
      <div class="flex flex-row w-full xl:px-24 mt-2 max-w-[1024px] m-auto items-center justify-around" :class="loading ? 'opacity-50' : null">
        <div class="hidden sm:flex w-full items-center justify-center">
          ...
          <icon-button
            class="mx-2"
            @click="() => adoptNewDNA({ dna: savedDNA[bookmark as Bookmarker] })"
            :disabled="(! canLoadDNA(savedDNA[bookmark as Bookmarker])) || (savedDNA[bookmark as Bookmarker] === dna)"
          >
      ...
      <tabs
        ...
        class="max-w-[1024px] m-auto w-full"
        nav-class="tab-nav border-b-2 flex flex-row mt-4 mb-2 md:items-stretch items-center justify-around"
      ...
      >
        ...
        <tab id="options_tab" name="Options" class="mb-4 flex flex-col items-center justify-start" :selected="true">
          ...
          <div class="flex w-full justify-center items-center flex-col xs:flex-row">
            <div class="flex flex-wrap md:w-2/3 justify-around">
              <div class="flex flex-col w-52 justify-start">
                <check-option id="transpose"   class="w-full"      :checked="options.transpose"     @change="updateTranspose">Swap Rows / Columns</check-option>
                <check-option id="smushStoned" class="w-full my-1" :checked="options.smushStoned"   @change="updateSmushStoned">Combine Similar Artifacts</check-option>
                <check-option id="showSidecar" class="w-full hidden lg:flex" :checked="options.showSidecar" @change="updateShowSidecar">Moar Fiddlers = BETTAR</check-option>
              </div>
              <div class="flex flex-col items-start justify-start">
                ...
                <div class="flex w-full flex-row items-center justify-center">
                  <input
                    id="boxMax"
                    :value.number="options.boxMax"
                    name="boxMax"
                    type="number"
                    :disabled="loading"
                    class="w-12 xs:w-16 md:w-20 max-h-32 self-center"
                    @input="updateBoxMax(($event.target as any).value)"
                  />
                  <div class="ml-2 text-sm">
                    <label for="boxMax" class="text-gray-600">Display Size Limit</label>
                  </div>
                </div>
              </div>
            </div>
            <div class="flex flex-row md:w-2/5 flex-wrap ml-2 items-center justify-between">
              <reset-button class="              w-[7rem]" @click="resetAllMaybe">Reset</reset-button>
              ...
              <template v-for="[id, dnaN] of Object.entries(savedDNA)" :key="id">
                <div class="flex py-2 md:flex-nowrap flex-wrap md:flex-row items-center justify-center contents-between">
                  ...
                  <div class="flex order-5 mt-2 sm:mt-0">
                    ...
                    <icon-button
                      class="w-24 md:w-20 mx-4 xs:mx-8 sm:mx-0"
                      :disabled="dnaN === dna"
        ...
        >
          <div class="flex flex-row w-full items-start justify-around">
            <div class="flex w-full items-center justify-center flex-col">
              <h3 class="flex mb-2 font-bold m-2 text-center">Groups</h3>
              <AspectOrderer :aspects="selaspects" :fancy="options.fancy" @update-order="updateAspectsLayout" @update-fancy="updateFancy" @reset-order="() => resetAxis('aspects')" @random-order="() => randomizeAxis('aspects')" />
            </div>
            ...
            <div class="hidden md:flex w-full mx-2 items-center content-center justify-center flex-col">
              ...
              <ArtifactOrderer :artifacts="artifacts" @update-order="updateArtifactsLayout" />
            ...
            <div class="hidden md:flex w-full items-center justify-center flex-col">
              ...
              <StoneOrderer :stones="stones" @update-order="updateStonesLayout" />
              <div class="h-full flex flex-col justify-end"><reset-button @click="() => resetAxis('stones')">Reset Stones</reset-button></div>
            </div>
          </div>
    ...
    <div>
      <tabs
        v-if="options.showSidecar"
        :options="{ useUrlFragment: false, defaultTabHash: 'grouping_tab' }"
        class="max-w-[14rem] mr-2 hidden lg:flex flex-col"
        nav-class="tab-nav border-b-2 flex 2xl:px-24 flex-row mb-2 md:items-stretch items-center justify-around"
        nav-item-class="tabs-nav grow flex py-2 px-2"
        nav-item-link-class="tabs-tab-nav flex w-full justify-center text-center"
        nav-item-active-class="tab-is-active bg-gray-200"
        nav-item-disabled-class="bg-red"
        panels-wrapper-class="min-h-[400px] tabs-panels"
      >
        <tab id="aspect_side_tab" name="Groups" class="p-2 mb-4 flex flex-col ordering">
          <div class="Aspects w-md mx-2 items-start content-center justify-start flex-col">
            <AspectOrderer :aspects="selaspects" :fancy="options.fancy" @update-order="updateAspectsLayout" @update-fancy="updateFancy" @reset-order="() => resetAxis('aspects')" @random-order="() => randomizeAxis('aspects')" />
          </div>
          <check-option id="showSidecar" class="ml-6 mt-1"  :checked="options.showSidecar" @change="updateShowSidecar">Hide Sidebar</check-option>
        </tab>
        <tab id="artifact_side_tab" name="Artifacts" class="p-2 mb-4 flex flex-col items-center ordering">
          <div class="Artifacts w-md mx-2 items-start content-center justify-start flex-col">
            <ArtifactOrderer  :artifacts="artifacts"  @update-order="updateArtifactsLayout" />
          </div>

        </tab>
        <tab id="stone_side_tab" name="Stones" class="Aspects p-2 mb-4 flex flex-col items-center ordering">
          <div class="Stones w-md mx-2 items-start content-center justify-start flex-col">
            <StoneOrderer     :stones="stones"        @update-order="updateStonesLayout" />
          </div>
        </tab>
...
import {
  ...
  storeSlotNames, loadSlotNames, LayoutAxis, PlayerDataOptions,
} from '@/lib'
...
import ArtifactOrderer from '@/components/ArtifactOrderer.vue';
import StoneOrderer    from '@/components/StoneOrderer.vue';
import AspectOrderer    from '@/components/AspectOrderer.vue';
...
const updateBoxMax = _.debounce((options, bx) => { options.boxMax = bx }, 1000)

export default defineComponent({
  components: {
    BookmarkIcon, CheckOption, CopyButton, DnaBox,
    HowThisWorks, IconButton, InventoryCanvas, ReplaceIcon,
    ResetButton, Tab, Tabs,
    ArtifactOrderer, AspectOrderer, StoneOrderer,
  },
  props: {
  ...
  data() {
    ...
    const res       = vivifyDNA(this.urldna || savedDNA.Bk1)
  ...
  computed: {
    ...
    clampedBoxMax() {
      const clampedBoxMax = Number(this.options.boxMax)
      if (clampedBoxMax > 0) {
        return _.clamp(clampedBoxMax, 128, 2048)
      }
      return 1280
    },
  },
  ...
  methods: {
    ...
    updateShowSidecar(event: Event) {
      this.options.showSidecar = (<HTMLInputElement>event.target)?.checked || false
    },
    updateFancy(fancy: boolean) {
      this.options.fancy = fancy // (<HTMLInputElement>event.target)?.checked || false
      this.aspects = { ...this.aspects }
    },
    updateBoxMax(boxMax: string) {
      updateBoxMax(this.options, boxMax)
    },
    ...
    saveCurrDNA(toBookmark: Bookmarker | string, old: DnaStr) {
      ...
      const _result = saveDNA(this.dna, effYouTS)
